...
Literal.prototype.getText = function() {
  // decode UTF8
  let text = util.decode_utf8(util.Uint8Array2str(this.data));
  // normalize EOL to \n
  return text.replace(/\r\n/g, '\n');
};
...
Literal.prototype.read = function(bytes) {
  ...
  let format = enums.read(enums.literal, bytes[0]);

  let filename_len = bytes[1];
  ...
  let data = bytes.subarray(6 + filename_len, bytes.length);
...
Literal.prototype.write = function() {
  let filename = util.str2Uint8Array(util.encode_utf8(this.filename));
  let filename_length = new Uint8Array([filename.length]);

  let format = new Uint8Array([enums.write(enums.literal, this.format)]);
  let date = util.writeDate(this.date);
  let data = this.getBytes();
