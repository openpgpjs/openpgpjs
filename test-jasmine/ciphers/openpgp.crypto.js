describe("Functional tests: openpgp_crypto_* methods", function () {
    ...
    it("RSA sign and verify", function () {
        // RSA
        var RSAsignedData = openpgp_crypto_signData(2, 1, RSApubMPIs, RSAsecMPIs, "foobar");
        var RSAsignedDataMPI = new openpgp_type_mpi();
        var result = openpgp_crypto_verifySignature(1, 2, [RSAsignedDataMPI.read(RSAsignedData, 0, RSAsignedData.length)], RSApubMPIs, "foobar");
        expect(result).toBeTruthy();
    });

    // DSA 
    it("DSA sign and verify", function () {
        ...
        var result = openpgp_crypto_verifySignature(17, 2, DSAmsgMPIs, DSApubMPIs, "foobar");
        expect(result).toBeTruthy();
    });
    ...
    describe("AES256", function () {
        ...
        it("Symmetric encrypt and decrypt with OpenPGP CFB resync", function () {
            var symmencDataOCFB = openpgp_crypto_symmetricEncrypt(openpgp_crypto_getPrefixRandom(symmAlgo), symmAlgo, symmKey, "foobar", true);
            var d1 = openpgp_crypto_symmetricDecrypt(symmAlgo, symmKey, symmencDataOCFB, true);
            expect(d1).toBe("foobar");
        });

        it("Symmetric encrypt and decrypt without OpenPGP CFB resync (used in modification detection code \"MDC\" packets)", function () {
            var symmencDataCFB = openpgp_crypto_symmetricEncrypt(openpgp_crypto_getPrefixRandom(symmAlgo), symmAlgo, symmKey, "foobar", false);
            var d1 = openpgp_crypto_symmetricDecrypt(symmAlgo, symmKey, symmencDataCFB, false);
            expect(d1).toBe("foobar");
        });

        it("Asymmetric encrypt and decrypt using RSA with eme_pkcs1 padding", function () {
            ...
            var d = openpgp_encoding_eme_pkcs1_decode(openpgp_crypto_asymetricDecrypt(1, RSApubMPIs, RSAsecMPIs, [RSAEncryptedDataMPI]).toMPI().substring(2), RSApubMPIs[0].mpiByteLength);
            expect(d).toBe(symmKey);
        });

        it("Symmetric encrypt and decrypt using Elgamal with eme_pkcs1 padding", function () {
            ...
            var d = openpgp_encoding_eme_pkcs1_decode(openpgp_crypto_asymetricDecrypt(16, ElgamalpubMPIs, ElgamalsecMPIs, ElgamalEncryptedDataMPIs).toMPI().substring(2), ElgamalpubMPIs[0].mpiByteLength);
            expect(d).toBe(symmKey);
        });
    });
