describe("AES Rijndael cipher test with test vectors from ecb_tbl.txt", function() {
    ...
    function test_aes(input, key) {
        return util.hexstrdump(util.bin2str(AESencrypt(input, keyExpansion(util.bin2str(key)))));
	...
	describe("128-bit key vectors", function () {

	    for (i = 0; i < testvectors128.length; i++) {

	        var block = testvectors128[i][1];
	        var key = testvectors128[i][0];
	        var expected = testvectors128[i][2];

	        it("block: " + util.hexidump(block)
                + " key: " + util.hexidump(key), function () {

                var expectedHex = util.hexstrdump(util.bin2str(expected));
                var res2 = util.hexstrdump(util.bin2str(AESencrypt(block, keyExpansion(util.bin2str(key)))));
	...
	describe("192-bit key vectors", function () {

	    for (i = 0; i < testvectors192.length; i++) {

	        var block = testvectors192[i][1];
	        var key = testvectors192[i][0];
	        var expected = testvectors192[i][2];
	...
	describe("256-bit key vectors", function () {

	    for (i = 0; i < testvectors256.length; i++) {

	        var block = testvectors256[i][1];
	        var key = testvectors256[i][0];
	        var expected = testvectors256[i][2];
