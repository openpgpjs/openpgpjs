describe("Twofish test with test vectors from http://www.schneier.com/code/ecb_ival.txt", function() {
	...
	for (var i = 0; i < 49; i++) {
		...
		if (i === 0) {
			...
			it("block: " + util.hexidump(blk) + " key: " + util.hexstrdump(key), function () {
			    expect(util.bin2str(TFencrypt(blk, key))).toBe(util.bin2str(ct));
			});
		} else if (i === 1) {
			blk = testvectors[0];
			key = util.bin2str(start);
