  ...
  describe('Encrypt and decrypt', function () {
    ...
    async function testCFB(plaintext) {
      await Promise.all(symmAlgos.map(async function(algo) {
        ...
        const symmencData = await crypto.cfb.encrypt(algo, symmKey, util.stringToUint8Array(plaintext), IV, openpgp.config);
        const text = util.uint8ArrayToString(await crypto.cfb.decrypt(algo, symmKey, symmencData, new Uint8Array(crypto.cipher[algo].blockSize)));
