...
-----END PGP PRIVATE KEY BLOCK-----`;
  ...
  describe('subkey', function(){
    ...
    it('create and add a new rsa subkey to a rsa key', async function() {
      const privateKey = (await openpgp.key.readArmored(priv_key_rsa)).keys[0];
      await privateKey.decrypt('hello world');
      console.log('getAlgorithmInfo', privateKey.getAlgorithmInfo());
      ...
      console.log('subkey.getAlgorithmInfo', newSubkey.getAlgorithmInfo());
    ...
    it('sign/verify data with the new subkey correctly', async function() {
      ...
      const signed = await openpgp.sign({message: openpgp.cleartext.fromText('the data to signed'), privateKeys: privateKey, armor:false});
      ...
      // console.log(await verified[0].verified);
    ...
    it('encrypt/decrypt data with the new subkey correctly', async function() {
      ...
      expect(await streamToStr(decrypted.getText())).to.be.equal(vData);
    });
  })
...
function streamToStr(stream, enc) {

  let result = '';
  let reader = stream.getReader();

  return reader.read().then(function processText(data) {
    if (!data.done) {
      result += data.value;
